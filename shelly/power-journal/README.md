...
nor the archive. The switch is dead while the plug cannot be reached, because
there is nothing to change without it.

The bytes the app uploads are the same ones `tools/upload.js` sends. They are
generated by `node tools/asset.js` into `app/src/main/assets/` and checked in,
because a Gradle build has no Node to run the minifier with; the test suite
compares the two, so a forgotten regeneration fails rather than ships.

What the app copies, it keeps. All four tiers are fetched, not just the finest,
and nothing is ever deleted locally. That is the point of copying at all: a
quarter hour that has fallen off the fine pages survives here at quarter hour
resolution long after the plug has it only as an hour, and then only as a day.
Reading lays the tiers over one another finest first, so a coarse block speaks
only for stretches no finer one described.

The fetch runs hourly in the background on an unmetered network, not only when
someone opens the screen. The native tier is one page — hours on a busy load —
so an app that fetched on demand would keep native detail for the last few
hours and never for anything older, however often it was opened.
